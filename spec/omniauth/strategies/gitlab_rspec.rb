...
describe OmniAuth::Strategies::GitLab do
  ...
  context "client options" do
    it 'should have correct site' do
      subject.options.client_options.site.should eq("https://gitlab.com/api")
    end

    it 'should have correct authorize url' do
      subject.options.client_options.authorize_url.should eq('https://gitlab.com/oauth/authorize')
    end

    it 'should have correct token url' do
      subject.options.client_options.token_url.should eq('https://gitlab.com/login/access_token')
    end

    describe "should be overrideable" do
      it "for site" do
        gitlab.options.client_options.site.should eq(gitlab_site)
      end

      it "for authorize url" do
        gitlab.options.client_options.authorize_url.should eq(gitlab_authorize_url)
      end

      it "for token url" do
        gitlab.options.client_options.token_url.should eq(gitlab_token_url)
      end
    end
  end
  ...
  context "#emails" do
    it "should use relative paths" do
      access_token.should_receive(:get).with('user/emails', :headers=>{"Accept"=>"application/vnd.gitlab.v3"}).and_return(response)
      subject.options['scope'] = 'user'
